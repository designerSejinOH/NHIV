...
export async function POST(request: NextRequest) {
  try {
    ...
    const { id, code, name_kr, name_en, sort_order } = await request.json()

    if (!id) {
      return NextResponse.json({ error: 'IUCN 등급 ID가 필요합니다.' }, { status: 400 })
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'IUCN 등급이 수정되었습니다.',
      data,
    })
  } catch (error) {
    console.error('Update IUCN status error:', error)
    return NextResponse.json({ error: 'IUCN 등급 수정 중 오류가 발생했습니다.' }, { status: 500 })
  }
}
